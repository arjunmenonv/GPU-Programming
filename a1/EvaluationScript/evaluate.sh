...
current_dir=$(pwd)
INPUT="$current_dir/Evaluation/Testcases/sampleSmall/Input/"
OUTPUT="$current_dir/Evaluation/Testcases/sampleSmall/Output/"
...
do
	...
	for testcase in $INPUT/*; do 
		...
		diff -w "$OUTPUT/$file_name" "cpu_seqloop.txt" > /dev/null 2>&1
		exit_code=$?
		if (($exit_code == 0)); then
		  	echo "success" >> $LOGFILE
		else 
